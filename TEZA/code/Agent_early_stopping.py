...
class DQNAgent(object):
    ...
    def sample_memory(self):
        ...
        #new for early stoping- seprate into train and validation?? its the same data? 
        a=int(self.batch_size/2)
        train_states = states[:a]
        train_rewards = rewards[:a]
        train_dones = dones[:a]
        train_actions = actions[:a]
        train_states_ = states_[:a]
        validation_states = states[a:]
        validation_rewards = rewards[a:]
        validation_dones = dones[a:]
        validation_actions = actions[a:]
        validation_states_ = states_[a:]

        return train_states, train_actions, train_rewards, train_states_, train_dones ,validation_states, validation_actions,validation_rewards, validation_states_, validation_dones
    ...
    def learn(self):
        ...
        train_states, train_actions, train_rewards, train_states_, train_dones ,validation_states, validation_actions,validation_rewards, validation_states_, validation_dones = self.sample_memory()
        a=self.batch_size/2
        indices = np.arange(a)


        
        q_pred = self.q_eval.forward(train_states)[indices, train_actions]
        q_next = self.q_next.forward(train_states_).max(dim=1)[0]

        # chaneged16.5 if done i dont want to change it tp 0 in this case there is no meaning for done
        # q_next[dones] = 0.0
        q_target = train_rewards + self.gamma * q_next
        ...
        self.q_eval.scheduler.step(loss_validation)
