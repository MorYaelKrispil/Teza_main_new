...
def init_test_param():
    ...
    episod_num_iter = round((len(Train.index) / episode_size))
    print('episod_num_iter!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!',episod_num_iter)
    ...
    save_path_test = path_pycharm + 'config changing tests/Trials/' + Tfname + '.csv'
...
def test_all_networks_main_union(data_num_rows_for_test, is_all_NN):
    ...
    if is_all_NN == True:
        ...
        for i in range(len(model_num_iter_array)):
            ...
            while not episode_instance.done_episode:
                ...
                p = i  # p=0
                dictionary = episode_instance.close_episode(save_path_test, test_result_df,
                                                            current_state_for_model_error_checks, p,
                                                            Best_param_train_in_iter, maxQ,episod_num_iter)
                name = agent.env_name + global_test_name
                save_dict_to_pikle(dictionary, name)
    ...
    if is_all_NN == False:
        ...
        while not episode_instance.done_episode:
            ...
            episode_last_error = episode_instance.episode_error
            p = 0
            print('episod_num_iter 55555555555555', episod_num_iter)
...
if __name__ == '__main__':
    ...
    path_pycharm = '/home/mor/project/TEZA/' #'/Users/zlililaor/Desktop/MorTeza-main/TEZA/'  # '/home/mor/project/TEZA/'
    global_algo = 'DQNAgent'
    global_env_name = 'FasionMnist on server'
    global_test_name = 'FasionMnist last NN'
    learner_model = 'RF_Classifier'  # 'ExtraTreesClassifier'#'DT'#'RF_Classifier'#'RF_Regressor' #'gbdt'#'RF_Regressor'
    ...
    train_main()
    init_test_param()
    # test_all_networks_main()  # test_last_networks_main()
    ...
    data_num_rows_for_test = 4000  # number of rows that we will check test error on for QA
